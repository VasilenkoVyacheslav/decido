...
function ppom_woocommerce_show_fields() {
    ...
    if( ppom_load_bootstrap_css() ) {
        ...
        $ppom_bs_js_cdn  = '//stackpath.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js';
		...
		foreach($option_prices as $option){
			...
			switch ($option['apply']) {
				...
				case 'measure':
					...
					$ppom_item_org_price	= $option['price'];
					$ppomm_measures			= $option['measure'];
					// $total_option_price = $total_option_price * $option['measure'];
...
function ppom_woocommerce_order_value( $display_value, $meta=null, $item=null ) {
	...
	switch( $input_type ) {
		...
		default:
			...
			$display_value = apply_filters('ppom_order_display_value', $display_value);
			break;
